var getCaseInfo = function () {
    console.log("这个函数调用后端的一个case信息过来！");
    var url = window.location.href;
    var userId = url.split("/")[5];

    $.get("/api/get_case_info/" + userId + "/", {},
        function (resp) {
            if (resp.code === 200) {
                ...
                // $(".parameterEditor").val(resp.data.request_body);
                ...
                //项目，模块
                // $("#selectProject").val(resp.data.project);
                // $("#selectModule").val(resp.data.module);
                // // SelectInit(resp.data.project,resp.data.module);
                // console.log("resp.data.project-->",resp.data.project);
                // console.log("resp.data.module-->",resp.data.module);

                //参数类型
                // console.log('resp.data.request_type-->',resp.data.request_type);
                if (resp.data.request_type === 1) {
                    $("#data").prop("checked", true);
                } else if (resp.data.request_type === 2) {
                    $("#json").prop("checked", true);
                } else {
                    window.alert("不支持该参数类型");
                }
